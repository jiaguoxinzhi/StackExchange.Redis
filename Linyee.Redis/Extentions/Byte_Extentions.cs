    ...
    public static class Byte_Extentions
    {
        ...
        public static byte[] BytesFromHexString(this string hex)
        {
            ...
            for(var spi = 0; spi < hexstr.Length; spi += 2)
            {
                ...
                byte b = byte.Parse(bhex, Globalization.NumberStyles.HexNumber);
                list.Add(b);
            }
            return list.ToArray();
        }
        ...
        /// <returns></returns>
        public static string ToHexString(this byte[] buf, int start = 0, int len = -1,int lineBytes=32)
        {
            var tlen = len;
            ...
            for (var fi = start; fi < start + tlen; fi++)
            {
                ...
                if ((fi + 1) % lineBytes == 0) sbd.AppendLine();
            }
        ...
        internal static uint ToUInt32Big(Stream stream, ref long offset,int len=4)
        {
            ...
            var res = BitConverter.ToUInt32(list.ToArray(),4);
            return res;
        }
        ...
        internal static int ToInt32Big(Stream stream, ref long offset, int len = 4)
        {
            ...
            list.AddRange(ubytes);
            list.AddRange(new byte[] { 0, 0, 0, 0 });
            var res = BitConverter.ToInt32(list.ToArray(),4);
            return res;
        }
        ...
        /// <param name="offset"></param>
        /// <returns></returns>
        internal static byte ToByte(Stream stream, ref long offset)
        {
        ...
        /// <param name="len"></param>
        /// <returns></returns>
        internal static byte[] GetBytes(Stream stream, int len)
        {
        ...
        /// <param name="len"></param>
        /// <returns></returns>
        internal static byte[] GetBytes(Stream stream,ref long offset, int len)
        {
            stream.Position = offset;
            byte[] buf = new byte[len];
            stream.Read(buf, 0, len);
